...
@click.option('--cache-dir', type=click.Path(path_type=Path),
              help='Directory to store intermediate files (default: $XDG_CACHE_HOME/hotcrp2pdf or ~/.cache/hotcrp2pdf)')
...
def cli(submissions_json: Path, output_pdf: Path, cache_dir: Path, 
    ...
    # Create converter
    converter = HotCRPConverter(cache_dir=cache_dir)
